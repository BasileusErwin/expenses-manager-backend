...
const createTransaction = [
	body("transactions", "")
		.if(body("type").not().exists())
		.isArray()
		.custom((value: CreateTransactionRequest[]) => {
			for (let data of value) {
				if (
					!Object.values(data).includes(Object.values(CreateTransactionRequest))
				) {
					return false;
				}
	...
	body("day", "Please enter a day")
		...
		.custom(
			(value: number, { req }) =>
				value > 0 && value <= dayHelper.getMaxDayByMonth(req.body.month),
		)
		.trim(),
	...
	body(
		"category.type",
		"Please enter a category type, and make it equal to the transaction type.",
	)
